...
/// Builder struct to create instance of [`TokensPricesRequestDetails`](crate::tokens::tokens_price::TokensPricesRequestDetails)
#[derive(Default)]
...
impl TokensPricesRequestBuilder {
    ...
    /// Attempts to construct a [`TokensPricesRequestDetails`](crate::tokens::tokens_price::TokensPricesRequestDetails) from the builder, returning errors if required fields are missing.
...
/// Represents the struct we receive after making request to get current prices for specified tokens.
/// In fact response is just a hashmap where key is token`s address and value its price in selected currency.
/// Note : 1inch always returns tokens addresses in lowercase.
