...
/// LiquidityProtocolsResponse is struct that defines object that server returns on /liquidity-sources request
#[derive(Deserialize, Debug)]
...
impl OneInchClient {
    /// Get current list of liquidity sources that are available for routing in 1inch.
    pub async fn get_liquidity_sources(
        &self,
    ) -> Result<LiquidityProtocolsResponse, Box<dyn Error>> {
        let url = format!(
        ...
        let liquidity_sources_response: LiquidityProtocolsResponse = response
            .json()
            .await
            .map_err(|e| Box::new(e) as Box<dyn Error>)?;
